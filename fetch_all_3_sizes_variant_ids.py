...
for h in handles:
    ...
    try:
        with urllib.request.urlopen(req) as resp:
            ...
            for v in variants:
                v_id = str(v.get("id"))
                title_val = str(v.get("title", ""))
                option1 = str(v.get("option1", ""))
...
print("=== COMPLETE VARIANT MAP FOR ALL 33 VARIANTS ===")
for handle, sizes in variant_map.items():
    print(f"  {handle}: {sizes}")
...
print("\nSUCCESS: Updated CartDrawer.tsx with 100% real numeric Shopify Variant IDs!")
