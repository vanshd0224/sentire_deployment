...
export const SHOPIFY_VARIANT_MAP: Record<string, Record<number, string>> = {
  ...
  "discovery-set-package": {
    36: "46965136031905",
    0: "46965136031905",
    50: "46965136031905",
  },
  "sentire-discovery-coffret-6x6ml": {
    36: "46965136031905",
    0: "46965136031905",
    50: "46965136031905",
  },
...
export const resolveShopifyVariantId = (item: any): string => {
  ...
  const pName = String(item?.name || "")
    ...
    .trim();
  const sizeNum =
    parseInt(String(item?.size || "").replace(/\D/g, ""), 10) || 50;
...
export const syncAddToCartToShopifyStorefront = async (
  item: any,
  quantity: number = 1,
) => {
  try {
    ...
    if (!existingCartId) {
      ...
      if (updatedCart?.checkoutUrl) {
        ...
        console.warn(
          "Stale Shopify Cart ID detected. Creating fresh cart...",
          userErrors,
        );
