      ...
      <div className="relative z-10 w-full max-w-6xl max-h-[94vh] md:max-h-[92vh] overflow-y-auto rounded-t-3xl md:rounded-3xl bg-[#fcfbf7] border-t md:border border-[#c89b5a]/40 shadow-[0_25px_80px_rgba(0,0,0,0.8)] text-[#1e1e1e] transition-all duration-300 hide-scrollbar glass-bottom-sheet md:glass-card-luxury">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 p-6 lg:p-10">
          ...
          <div className="lg:col-span-6 space-y-6">
            ...
            {/* ── LUXURY SCENT NOTE TAGS & PYRAMID VISUALIZER ── */}
            <div className="rounded-2xl border border-[#c89b5a]/30 bg-[#fdfbf7] p-5 space-y-4 shadow-sm">
              <div className="flex items-center justify-between border-b border-black/8 pb-3">
                <div>
                  <span className="text-[10px] font-bold uppercase tracking-[0.25em] text-[#c89b5a] block">
                    HAUTE OLFACTORY PYRAMID
                  </span>
                  <p className="text-[11px] text-[#1e1e1e]/60 mt-0.5">
                    Click any note tag to inspect olfactory composition
                  </p>
                </div>
                <span className="text-xs">✨</span>
              </div>

              {/* Scent Tiers: Top, Heart, Base */}
              <div className="space-y-3">
                {/* 1. Top Notes */}
                <div className="space-y-1.5">
                  <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-[#c89b5a]">
                    <span>TOP NOTES (0–15 MINS)</span>
                    <span className="h-[1px] flex-1 bg-[#c89b5a]/20" />
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {scentPyramid.top.map((note) => (
                      <button
                        key={note.name}
                        onClick={() => setSelectedNote(selectedNote?.name === note.name ? null : note)}
                        className={`flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] sm:text-xs font-semibold transition-all cursor-pointer touch-manipulation ${
                          selectedNote?.name === note.name
                            ? "border-[#c89b5a] bg-[#c89b5a] text-white shadow-md"
                            : "border-[#c89b5a]/30 bg-[#c89b5a]/10 text-[#1e1e1e] hover:border-[#c89b5a]"
                        }`}
                      >
                        <span>{note.icon}</span>
                        <span>{note.name}</span>
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-[#c89b5a]">
                    <span>HEART NOTES (15 MINS–4 HRS)</span>
                    <span className="h-[1px] flex-1 bg-[#c89b5a]/20" />
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {scentPyramid.heart.map((note) => (
                      <button
                        key={note.name}
                        onClick={() => setSelectedNote(selectedNote?.name === note.name ? null : note)}
                        className={`flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] sm:text-xs font-semibold transition-all cursor-pointer touch-manipulation ${
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-[#c89b5a]">
                    <span>BASE NOTES (4–16+ HRS)</span>
                    <span className="h-[1px] flex-1 bg-[#c89b5a]/20" />
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {scentPyramid.base.map((note) => (
                      <button
                        key={note.name}
                        onClick={() => setSelectedNote(selectedNote?.name === note.name ? null : note)}
                        className={`flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] sm:text-xs font-semibold transition-all cursor-pointer touch-manipulation ${
              ...
              {/* Interactive Scent Note Detail Popover Card */}
              {selectedNote && (
                <div className="mt-3 rounded-xl border border-[#c89b5a]/40 bg-gradient-to-br from-[#0d0906] to-[#1a140f] p-4 text-white shadow-lg animate-in fade-in slide-in-from-bottom-2">
                  <div className="flex items-center justify-between border-b border-white/10 pb-2 mb-2">
                    <div className="flex items-center gap-2">
                      <span className="text-xl">{selectedNote.icon}</span>
                      <div>
                        <h5 className="font-display font-semibold text-sm text-[#e2c48e]">{selectedNote.name}</h5>
                  ...
                  <p className="text-xs text-cream/80 leading-relaxed">{selectedNote.desc}</p>
                  <div className="mt-3 flex items-center justify-between text-[10px] font-semibold text-[#c89b5a]">
                    <span>Olfactory Intensity:</span>
                    <span>{selectedNote.intensity}% Extrait Strength</span>
                  </div>
                </div>
              )}
            </div>
