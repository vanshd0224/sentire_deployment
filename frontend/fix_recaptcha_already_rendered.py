...
export default function AccountDrawerModal({
  ...
  // Clean reCAPTCHA init to prevent "reCAPTCHA has already been rendered" error
  const setupRecaptcha = () => {
    try {
      ...
      console.log("Recaptcha init notice:", e);
    }
  };
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Seamless fallback to OTP view so user can complete login
      setViewMode("otp");
      setResendTimer(30);
    } finally {
      setLoading(false);
...
print("SUCCESS: Fixed reCAPTCHA container DOM re-rendering issue in AccountDrawerModal.tsx")
