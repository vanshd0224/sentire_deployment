...
chatbot_service_path = r"C:\Users\asus\.gemini\antigravity\scratch\sentire_deployment\backend\services\ai\chatbotService.js"

chatbot_code = """const { GoogleGenerativeAI } = require('@google/generative-ai');
...
class ChatbotService {
  ...
  _buildMasterSystemPrompt(userQuestion, productContext) {
    const coreElevenStr = sentireDataset.core_eleven_fragrances.map(f => 
      `- ${f.name} (${f.family}): ${f.desc} [Sizes: ${f.sizes.join(', ')}]`
    ).join('\n');
...
print("SUCCESS: Trained ChatbotService with Sentire 840 Q&A Dataset & Rules!")
