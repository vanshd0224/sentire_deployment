...
# Also copy all files from public/images/ directly into public/assets/ so both /images/ and /assets/ work 100%!
images_folder = os.path.join(pub_dir, "images")
assets_folder = os.path.join(pub_dir, "assets")

for root, dirs, files in os.walk(images_folder):
    for f in files:
        if f.lower().endswith(('.png', '.jpg', '.jpeg', '.webp')):
            src = os.path.join(root, f)
            dest = os.path.join(assets_folder, f)
            shutil.copyfile(src, dest)
            print(f"Copied {f} to assets folder!")

# 1. Update BestSellers.tsx with valid /assets/ image URLs
...
# 2. Update NewArrivals.tsx with valid /assets/ image URLs
...
# 3. Update PerfumesPage.tsx Purple Oud banner URL to /assets/purple-oud-banner.png?v=7
perfumes_page_path = os.path.join(src_dir, "components", "PerfumesPage.tsx")
with open(perfumes_page_path, "r", encoding="utf-8") as f:
    p_code = f.read()

p_code = p_code.replace('/assets/images/purple-oud-arrival.png?v=5', '/assets/purple-oud-banner.png?v=7')
p_code = p_code.replace('/assets/images/purple-oud-arrival.png', '/assets/purple-oud-banner.png?v=7')

with open(perfumes_page_path, "w", encoding="utf-8") as f:
    f.write(p_code)

print("SUCCESS: Updated image URLs to /assets/ with cache-buster ?v=7!")
