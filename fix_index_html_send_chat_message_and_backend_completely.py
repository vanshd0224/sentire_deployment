...
# 1. Purge any unparsed template string references from backend/services/ai/chatbotService.js
...
Respond intelligently as Gemini AI:`;
  ...
  _getDynamicGeminiReplicaFallback(message) {
    ...
    if (q.includes('engrav')) {
      return "Custom bottle laser engraving is complimentary on all 50 ML signature bottles! You can engrave initials, names, or special dates directly on the bottle.";
    }
...
print("SUCCESS: Purged all template literal escapes from backend/services/ai/chatbotService.js")

# 2. Purge index.html completely of any unparsed template string references
with open(index_path, 'r', encoding='utf-8') as f:
    html = f.read()

# Replace sendChatMessage block in index.html cleanly
p_start = html.find("async function sendChatMessage(text) {")
p_end = html.find("window.switchConciergeTab = function(tabName) {")
if p_end == -1:
    p_end = html.find("function switchConciergeTab(tabName) {")
...
if p_start != -1 and p_end != -1:
  ...
  """
    ...
    with open(index_path, 'w', encoding='utf-8') as f:
        f.write(html)
    print("SUCCESS: Updated index.html sendChatMessage cleanly")
