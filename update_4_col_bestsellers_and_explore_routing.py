...
# 1. Update BestSellers.tsx (4 products, 4 grid cols, Explore All opens BestSellersPage)
...
# 2. Update NewArrivals.tsx Explore button handler
...
# 3. Update App.tsx to pass onOpenBestSellersPage and onOpenNewArrivalsPage to BestSellers and NewArrivals components
app_path = os.path.join(src_dir, "App.tsx")
with open(app_path, "r", encoding="utf-8") as f:
    app_code = f.read()

# Replace <BestSellers ... /> call to include onOpenBestSellersPage={() => setActivePage("bestsellers")}
app_code = re.sub(
    r'<BestSellers\s+([^/>]*)\/>',
    r'<BestSellers \1 onOpenBestSellersPage={() => setActivePage("bestsellers")} />',
    app_code
)

# Replace <NewArrivals ... /> call to include onOpenNewArrivalsPage={() => setActivePage("new-arrivals")}
app_code = re.sub(
    r'<NewArrivals\s+([^/>]*)\/>',
    r'<NewArrivals \1 onOpenNewArrivalsPage={() => setActivePage("new-arrivals")} />',
    app_code
)
...
print("SUCCESS: BestSellers set to 4 products matching New Arrivals, and Explore All buttons open full BestSellersPage & NewArrivalsPage!")
